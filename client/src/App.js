...
// use below to read local JSON
// import { jsonResponse } from './users';
...
/*
const users = [
  {username: 'amy', password: '12345', isChatEnabled: true },
  {username: 'nick', password: '12345', isChatEnabled: false},
  {username: 'snowden', password: '12345', isChatEnabled: true}
];
...
class App extends Component {
  constructor(props) {
    ...
    //_users = jsonResponse.data.users;
  }
  ...
  readExcel() {
      ...
      oReq.onload = function(e) {
          ...
          _users = XLSX.utils.sheet_to_json(worksheet, {
              raw: true
          });
  ...
  loginSuccess = (result) => {
    window.sessionStorage.token = JSON.stringify(result.token);
    this.setState({
      showLogin: false,
      isChatEnabled: result.isChatEnabled
    });
  }
  ...
  handleFormSubmit(e) {
    ...
    $.ajax({
      ...
      success: function(result) {
        ...
        result.success ? that.loginSuccess(result) : that.loginError();
      },
      error: function(err) {
    ...
    const { isValidUser, isChatEnabled } = checkIfValidUser(credentials);
    if(isValidUser) {
      this.setState({
        showLogin: false,
        isChatEnabled
  ...
  showChatWindow() {
    ...
    this.showResponse({
      message: `Hi ${this.state.uname}. Welcome to Chatbot!!! Start your conversation`
    })
  }
    ...
    if (e.key === 'Enter') {
      ...
      if (inputText !== '') {
        /*this.showRequest(inputText);
          ...
          axios.defaults.headers.common['authorization'] = JSON.parse(window.sessionStorage.token);
          axios.get('/validateSession').then(response => {
            response.data.success ? this.showRequest(inputText) : this.setState({
              userInput: '',
              messages: [],
              inputEnabled: true,
              attempts: 0,
              ...
              isUserValid: false,
              isChatEnabled: false,
              notification: 'Your session has expired. Please login again.'
            });
          });
      }
  ...
  showRequest(inputText) {
    ...
    /*if (this.state.attempts === 0) {
      ...
      const { isValidUser, isChatEnabled } = checkIfValidUser(credentials);
      if (isValidUser) {
        obj.message = `Hi ${credentials.username}. Welcome to Chatbot!!! Start your conversation`;
      } else {
        ...
        {this.state.showLogin ?
          <div className="container">
            ...
            {this.state.isChatEnabled ? <React.Fragment>
              {this.state.showChatIcon ?
                ...
                <div style={styles.container} ref={this.slideBox}>
                  <div style={{height: 50, backgroundColor: 'yellow', padding: 5, color: 'green', fontSize: 22, position: 'fixed', width: 447, borderTopLeftRadius: 15, borderTopRightRadius: 15}}>
                    ...
                    <span style={{verticalAlign: 'top', margin: '0 10px'}}>Reporting Bot</span>
              ...
              </React.Fragment> : <div style={{position: 'absolute', top: 0, left: 0, width: '100%', color: 'red', border: 'red solid 1px', padding: 10, textAlign: 'center'}}>{msg}</div>}
          </div>
        }
      </React.Fragment>
    )
